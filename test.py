...
def createSTA(USER):
    for i in range(0, USER):
        ...
        sta.probability = get_backoff_probability(sta.traffic)  # 트래픽에 따른 백오프 시작 확률 설정
        stationList.append(sta)

def allocationRA_RU():
    for sta in stationList:
        if (sta.bo <= 0 and random.random() < sta.probability):  # 백오프 타이머가 0보다 작아졌고, 확률에 따라 백오프를 시작
